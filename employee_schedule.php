...
if (isset($_POST['action']) && $_POST['action'] === 'add_event') {
    ...
    $stmt->bind_param('iiisssssssi', $case_id, $selected_user_id, $client_id, $type, $title, $description, $date, $start_time, $end_time, $location, $employee_id);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        try {
            ...
            $details = "Created by: " . ($_SESSION['user_name'] ?? 'Employee') . "; Attorney: " . ($attorney_name !== '' ? $attorney_name : (string)$selected_user_id) . "; Client: " . ($client_name !== '' ? $client_name : ($client_id ? (string)$client_id : 'N/A')) . "; Title: $title; Type: $type; Date: $date; Time: $start_time-$end_time; Location: $location";
        ...
        if ($conn->query("SHOW TABLES LIKE 'notifications'")->num_rows) {
            ...
            $nMsg = "A new $type has been scheduled for you by employee: $employee_name - $title on $date from $start_time to $end_time at $location";
            $userType = 'attorney';
...
if (isset($_POST['action']) && $_POST['action'] === 'edit_event') {
    ...
    $stmt = $conn->prepare("UPDATE case_schedules SET type=?, title=?, description=?, date=?, start_time=?, end_time=?, location=? WHERE id=?");
    $stmt->bind_param('sssssssi', $type, $title, $description, $date, $start_time, $end_time, $location, $event_id);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        if ($schedule_info['attorney_id'] && $conn->query("SHOW TABLES LIKE 'notifications'")->num_rows) {
            ...
            $nMsg = "Your schedule has been updated by employee: $employee_name - $title on $date from $start_time to $end_time at $location";
            $userType = 'attorney';
            $notificationType = 'info';
...
if (isset($_POST['action']) && $_POST['action'] === 'add_walkin') {
    ...
    if (empty($client_middle_name)) {
        echo 'error: Middle name is required';
        exit;
    }
    ...
    $stmt = $conn->prepare("INSERT INTO case_schedules (case_id, attorney_id, client_id, walkin_client_name, walkin_client_contact, type, title, description, date, start_time, end_time, location, created_by_employee_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param('iiisssssssssi', $case_id, $selected_user_id, $client_id, $client_name, $client_contact, $type, $title, $description, $date, $start_time, $end_time, $location, $employee_id);
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            $details = "Created by: $employee_name; Attorney: " . ($attorney_name !== '' ? $attorney_name : (string)$selected_user_id) . "; Walk-in Client: $client_name (Contact: $client_contact); Title: $title; Type: $type; Date: $date; Time: $start_time-$end_time; Location: $location";
...
<body>
        ...
        <div class="modal-content add-event-modal">
            ...
            <div class="modal-body">
                <form id="walkinForm" class="event-form-grid">
                    ...
                    <div class="form-group">
                        <label for="walkinClientMiddleName">Middle Name <span style="color: red;">*</span></label>
                        <input type="text" id="walkinClientMiddleName" name="client_middle_name" required placeholder="Enter middle name">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="walkinTitle">Schedule Title <span style="color: red;">*</span></label>
                        <input type="text" id="walkinTitle" name="title" required placeholder="Enter schedule title">
                    </div>
                    <div class="form-group">
                        <label for="walkinDate">Date <span style="color: red;">*</span></label>
                        <input type="date" id="walkinDate" name="date" required>
                    </div>
                    <div class="form-group">
                        <label for="walkinStartTime">Start Time <span style="color: red;">*</span></label>
                        <input type="time" id="walkinStartTime" name="start_time" required>
                    </div>
                    <div class="form-group">
                        <label for="walkinEndTime">End Time <span style="color: red;">*</span></label>
                        <input type="time" id="walkinEndTime" name="end_time" required>
                    </div>
                    <div class="form-group">
    ...
    <script>
        ...
        function initializeCalendarFunctions(calendar) {
            ...
            if (saveEventBtn) {
                saveEventBtn.onclick = function() {
                    ...
                    if (!titleEl || !locationEl || !descriptionEl || !eventDateEl || !eventStartTimeEl || !eventEndTimeEl || !attorneySelect) {
        ...
        function showEventDetails(event) {
            ...
            set('modalTitle', event.extendedProps?.title || event.extendedProps?.type || 'Event');
