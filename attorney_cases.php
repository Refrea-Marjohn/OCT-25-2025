...
if (isset($_POST['action']) && $_POST['action'] === 'edit_case') {
    ...
    $stmt = $conn->prepare("SELECT title, client_id FROM attorney_cases WHERE id=? AND attorney_id=?");
...
<body>
    ...
    <div class="main-content">
        ...
        <?php 
        $page_title = 'My Cases';
        $page_subtitle = 'All cases you are handling as an attorney';
				...
				<div class="cases-grid" id="casesGrid">
					<?php foreach ($cases as $case): ?>
                            ...
                            <div class="case-card-footer">
                                <button class="btn-view-case" onclick="openCaseView(<?= $case['id'] ?>, '<?= htmlspecialchars($case['title'] ?? '') ?>', '<?= htmlspecialchars($case['client_name'] ?? '') ?>', '<?= htmlspecialchars($case['description'] ?? '') ?>', '<?= htmlspecialchars($case['status'] ?? '') ?>', '<?= htmlspecialchars($case['case_type'] ?? '') ?>', <?= $case['client_id'] ?>)">
                    ...
                    <div style="background: #f8f9fa; padding: 12px; border-radius: 8px; margin-bottom: 12px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 12px;">
                            ...
                            <div style="background: white; padding: 8px; border-radius: 6px; border-left: 3px solid #28a745;">
                                <h5 style="margin: 0 0 4px 0; color: #28a745; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.3px;">Case Type</h5>
                                <p style="margin: 0; color: #2c3e50; font-weight: 500; font-size: 0.9rem;">${caseType || 'Not specified'}</p>
                            </div>
                        </div>
