...
if (empty($title) || empty($date) || empty($start_time) || empty($end_time) || empty($location) || empty($type)) {
...
$stmt = $conn->prepare("UPDATE case_schedules SET title = ?, date = ?, start_time = ?, end_time = ?, location = ?, type = ?, description = ? WHERE id = ?");
if (!$stmt) {
...
$stmt->bind_param("sssssssi", $title, $date, $start_time, $end_time, $location, $type, $description, $event_id);

error_log("About to execute statement");
if ($stmt->execute()) {
    // Log the action
    $scheduleDetails = "Event ID: $event_id, Title: $title, Date: $date, Start Time: $start_time, End Time: $end_time, Location: $location, Type: $type";
