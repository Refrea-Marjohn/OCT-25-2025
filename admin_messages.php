...
if (isset($_POST['action']) && $_POST['action'] === 'check_seen_status') {
    ...
    // Check if this conversation is already marked as seen (same as attorney messages)
    $stmt = $conn->prepare("SELECT seen_status FROM client_attorney_assignments WHERE id = ?");
    $stmt->bind_param('i', $conversation_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
// Handle AJAX mark messages as seen
if (isset($_POST['action']) && $_POST['action'] === 'mark_messages_seen') {
    $conversation_id = intval($_POST['conversation_id']);
    
    // Update seen status (same as attorney messages)
    $stmt = $conn->prepare("UPDATE client_attorney_assignments SET seen_status = 'Seen' WHERE id = ?");
    $stmt->bind_param('i', $conversation_id);
    $stmt->execute();
    ...
    if ($result === 'success') {
        ...
        // Notify client that admin has seen their conversation (only if status was changed)
...
<body>
    ...
    <script>
        ...
        function markAsSeen() {
            ...
            const fd = new FormData();
            fd.append('action', 'mark_messages_seen');
            fd.append('conversation_id', selectedConversationId);
            ...
            fetch('admin_messages.php', { method: 'POST', body: fd })
                ...
                .then(result => {
                    if (result === 'success') {
                        ...
                        fetchMessages(); // Refresh messages to remove unseen indicators
        ...
        function updateMarkAsSeenButton(isSeen) {
            ...
            if (markSeenBtn) {
                if (isSeen) {
                    ...
                    markSeenBtn.innerHTML = '<i class="fas fa-eye"></i> Mark as Seen';
                    markSeenBtn.className = 'btn btn-success';
