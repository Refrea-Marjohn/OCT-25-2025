...
// Handle AJAX check seen status
if (isset($_POST['action']) && $_POST['action'] === 'check_seen_status') {
    $conversation_id = intval($_POST['conversation_id']);
    
    // Check if this conversation is already marked as seen
    $stmt = $conn->prepare("SELECT seen_status FROM client_attorney_assignments WHERE id = ? AND attorney_id = ?");
    $stmt->bind_param('ii', $conversation_id, $attorney_id);
    $stmt->execute();
    $result = $stmt->get_result();
    
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        echo ($row['seen_status'] === 'Seen') ? 'seen' : 'unseen';
    } else {
        echo 'unseen';
    }
    exit();
...
// Handle AJAX mark as seen
if (isset($_POST['action']) && $_POST['action'] === 'mark_as_seen') {
    $conversation_id = intval($_POST['conversation_id']);
    
    // Update seen status
    $stmt = $conn->prepare("UPDATE client_attorney_assignments SET seen_status = 'Seen' WHERE id = ? AND attorney_id = ?");
    $stmt->bind_param('ii', $conversation_id, $attorney_id);
    $stmt->execute();
    
    // Check if query executed successfully (regardless of affected rows)
    $result = $stmt->error ? 'error' : 'success';
    
    if ($result === 'success') {
        // Check if status was actually changed (to avoid duplicate notifications)
        $was_changed = $stmt->affected_rows > 0;
        ...
        $auditLogger->logAction(
            ...
            'Mark as Seen',
            'Communication',
            "Marked client conversation as seen - Assignment ID: $conversation_id",
            'success',
            'low'
        );
        
        // Notify client that attorney has seen their conversation (only if status was changed)
        if ($was_changed && $conn->query("SHOW TABLES LIKE 'notifications'")->num_rows) {
            // Get client info
            $stmt_client = $conn->prepare("SELECT client_id FROM client_attorney_assignments WHERE id = ?");
            $stmt_client->bind_param('i', $conversation_id);
            $stmt_client->execute();
...
<body>
            ...
            <div class="chat-area">
                <div class="chat-header">
                    ...
                    <div class="chat-actions" id="chatActions" style="display:none;">
                        <button class="btn btn-success" id="markAsSeenBtn" onclick="markAsSeen()">
    ...
    <script>
        ...
        function selectClient(conversationId, clientName, clientId) {
            ...
            // Check seen status and update button
            checkMessagesSeenStatus();
        ...
        function markAsSeen() {
            ...
            const fd = new FormData();
            fd.append('action', 'mark_as_seen');
            fd.append('conversation_id', selectedConversationId);
            
            fetch('attorney_messages.php', { method: 'POST', body: fd })
                .then(r => r.text()).then(res => {
                    if (res === 'success') {
                        // Update button to show seen status
                        updateMarkAsSeenButton(true);
                        
                        // Show success message
                        showNotification('Messages marked as seen successfully!', 'success');
                    } else {
                        showNotification('Error marking messages as seen', 'error');
                    }
                })
                .catch(error => {
        ...
        function updateMarkAsSeenButton(isSeen) {
            const markSeenBtn = document.getElementById('markAsSeenBtn');
            if (markSeenBtn) {
                if (isSeen) {
                    markSeenBtn.innerHTML = '<i class="fas fa-check-circle"></i> Message Seen';
                    markSeenBtn.className = 'btn btn-success';
                    markSeenBtn.disabled = true;
                } else {
                    markSeenBtn.innerHTML = '<i class="fas fa-check"></i> Mark as Seen';
        ...
        function checkMessagesSeenStatus() {
            if (!selectedConversationId) return;
            
            const fd = new FormData();
            fd.append('action', 'check_seen_status');
            fd.append('conversation_id', selectedConversationId);
            
            fetch('attorney_messages.php', { method: 'POST', body: fd })
                .then(r => r.text())
                .then(result => {
                    const isSeen = result === 'seen';
                    updateMarkAsSeenButton(isSeen);
                })
                .catch(error => {
